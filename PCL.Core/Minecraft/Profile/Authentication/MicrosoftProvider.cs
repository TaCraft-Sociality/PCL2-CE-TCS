    ...
    public MicrosoftProvider(string? clientId = null)
    {
        ...
        if (string.IsNullOrWhiteSpace(_clientId))
            throw new InvalidOperationException(
                "未配置微软登录的 OAuth Client ID：请改用离线登录或第三方验证账号； " +
                "如需微软正版登录，请设置环境变量 PCL_MS_CLIENT_ID 后重启启动器。 " +
                "Microsoft OAuth client id is not configured. Use an offline / third-party account, " +
                "or set the PCL_MS_CLIENT_ID environment variable and restart.");
    }
