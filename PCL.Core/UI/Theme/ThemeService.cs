...
public sealed partial class ThemeService
{
    ...
    {
        // TCS 定制版：只把「不在可选列表里的旧配置」（如原版天空蓝等）纠正为 TCS 橙；
        // 用户主动选择的死机蓝 / 龙猫蓝会在重启后保留。
        if (!SelectableThemes.Contains(CurrentTheme)) CurrentTheme = ColorTheme.CatBlue;
    ...
    public static (int Hue, double LightAdjust, double ChromaAdjust) GetCurrentThemeArgs()
    {
        ...
        return theme switch
        {
            ...
            // TCS 服务器主题：橙色（LCH 色相约 55），略提亮、略提纯度
            ColorTheme.TcsOrange => (55, 0.06, 0.28),
#if DEBUG
            _ => ((int)theme, 0, 0)
#else
    ...
    /// <summary>
    ///     当前可供用户选择的配色主题列表（TCS 橙 + 死机蓝 + 龙猫蓝）。
    /// </summary>
    public static ColorTheme[] SelectableThemes => [ColorTheme.SkyBlue, ColorTheme.CatBlue, ColorTheme.DeathBlue];

    /// <summary>
    ///     将配色主题转换为下拉框下标。
    /// </summary>
    public static int ThemeToIndex(ColorTheme theme)
    {
        var index = Array.IndexOf(SelectableThemes, theme);
    ...
    ///     将下拉框下标转换为配色主题。
    /// </summary>
    public static ColorTheme IndexToTheme(int index) =>
        index >= 0 && index < SelectableThemes.Length ? SelectableThemes[index] : SelectableThemes[0];
