...
public partial class FormMain
{
...
#if DEBUG
        if (ModTcsSelfTest.IsEnabled)
        {
            // TCS 定制版自检（PCL_TCS_SELFTEST=1 时启用）
            ModTcsSelfTest.PrepareEnvironment();
            Loaded += (_, _) => ModTcsSelfTest.RunAsync();
        }
#endif
    ...
    private void FormMain_SizeChanged(object? sender = null, EventArgs? e = null)
    {
        ...
        if (PanBack is not null)
        {
            ...
            // TCS 定制：主导航栏已移到左侧第 0 列，PanMain 位于第 1 列，
            // 宽度必须扣掉导航栏宽度，否则会超出右边界（界面右侧突出、内容被裁切）。
            var navWidth = 0d;
            if (PanNav is not null && PanNav.Visibility == Visibility.Visible)
                navWidth = PanNav.ActualWidth > 0d
                    ? PanNav.ActualWidth
                    : double.IsNaN(PanNav.Width) ? 0d : PanNav.Width; // 布局完成前用设定的宽度
            PanMain.Width = Math.Max(0d, formWidth - navWidth);
    ...
    public void PageChange(PageStackData stack, PageSubType subType = PageSubType.Default)
    {
        if (string.IsNullOrEmpty(PageNameGet(stack)))
        {
            ...
            // TCS 定制：导航栏改为可扩展的竖排列表，不能再按固定下标取按钮，改为按 Tag 匹配
            foreach (var child in PanTitleSelect.Children)
                if (child is MyRadioButton navButton && navButton.Tag?.ToString() == ((int)stack.page).ToString())
                {
                    navButton.SetChecked(true, true, string.IsNullOrEmpty(PageNameGet(pageCurrent)));
                    break;
                }
    ...
    private void PageChangeActual(PageStackData stack, PageSubType subType)
    {
        ...
        try
        {
            ...
            switch (stack.page)
            {
                ...
                case PageType.Community: // TCS 社区
                    {
                        if (ModMain.frmTcsCommunity is null)
                        {
                            ModMain.frmTcsCommunity = new PageTcsBrowser("https://bbs.tcscraft.com", "TCS 社区");
                            ModMain.frmTcsCommunityLeft = new PageTcsLeft("TCS 社区", ModMain.frmTcsCommunity);
                        }

                        if (ModMain.frmTcsCommunityLeft is null)
                            ModMain.frmTcsCommunityLeft = new PageTcsLeft("TCS 社区", ModMain.frmTcsCommunity);
                        PageChangeAnim(ModMain.frmTcsCommunityLeft, ModMain.frmTcsCommunity);
                        break;
                    }
                case PageType.Skin: // TCS 皮肤站
                    {
                        if (ModMain.frmTcsSkin is null)
                        {
                            ModMain.frmTcsSkin = new PageTcsBrowser("https://tcsskins.tcscraft.com/", "TCS 皮肤站");
                            ModMain.frmTcsSkinLeft = new PageTcsLeft("TCS 皮肤站", ModMain.frmTcsSkin);
                        }

                        if (ModMain.frmTcsSkinLeft is null)
                            ModMain.frmTcsSkinLeft = new PageTcsLeft("TCS 皮肤站", ModMain.frmTcsSkin);
                        PageChangeAnim(ModMain.frmTcsSkinLeft, ModMain.frmTcsSkin);
                        break;
                    }
                case PageType.Bili: // TCS B站官方
                    {
                        if (ModMain.frmTcsBili is null)
                        {
                            ModMain.frmTcsBili = new PageTcsBrowser("https://space.bilibili.com/40462438", "TCS B站官方");
                            ModMain.frmTcsBiliLeft = new PageTcsLeft("TCS B站官方", ModMain.frmTcsBili);
                        }

                        if (ModMain.frmTcsBiliLeft is null)
                            ModMain.frmTcsBiliLeft = new PageTcsLeft("TCS B站官方", ModMain.frmTcsBili);
                        PageChangeAnim(ModMain.frmTcsBiliLeft, ModMain.frmTcsBili);
                        break;
                    }
                case PageType.Link: // TCS 联机（陶瓦联机）
                    {
                        if (ModMain.frmTcsLink is null)
                        {
                            ModMain.frmTcsLink = new PageTcsBrowser(ModTerracotta.PrepareHintPage(), "陶瓦联机", showToolbar: false);
                            ModMain.frmTcsLink.InjectedCss = PageTcsLinkLeft.BuildSkinCss();
                        }
                        if (ModMain.frmTcsLinkLeft is null)
                            ModMain.frmTcsLinkLeft = new PageTcsLinkLeft();
                        if (!string.IsNullOrWhiteSpace(ModTerracotta.UiUrl))
                            ModMain.frmTcsLink.NavigateTo(ModTerracotta.UiUrl);
                        PageChangeAnim(ModMain.frmTcsLinkLeft, ModMain.frmTcsLink);
                        break;
                    }
