    ...
    private void GetAnnouncement()
    {
        ModBase.RunInNewThread(() =>
        {
            try
            {
                ...
                // TCS 定制：联机服务根地址来自官方 CI 注入的密钥（LINK_SERVER_ROOT），
                // 本地构建/定制分发时为空字符串，会拼出相对 URI 并抛 InvalidOperationException，
                // 反复弹出「发生意外错误」窗口。这里直接优雅退出，不再抛异常。
                if (!Secrets.LinkServers.Any(server =>
                        Uri.TryCreate(server?.Trim(), UriKind.Absolute, out _)))
                {
                    LogWrapper.Info("[Link] 未配置联机服务器地址（LINK_SERVER_ROOT 为空），已跳过联机公告获取");
                    ModBase.RunInUi(() =>
                    {
                        HintAnnounce.Theme = MyHint.Themes.Yellow;
                        HintAnnounce.Text = "本启动器未启用联机功能";
